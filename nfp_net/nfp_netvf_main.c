...
/* The offset of the VF queues max queues and VF id */
#define NFP_NET_VF_OFFSET(_maxqs, _vfid) (0x800 * 2 * _maxqs * _vfid)
#endif
			       ...
			       const struct pci_device_id *pci_id)
{
	...
	err = dma_set_mask(&pdev->dev, DMA_BIT_MASK(NFP_NET_MAX_DMA_BITS));
	if (err)
		goto err_dma_mask;
	...
	case PCI_DEVICE_NFP6000VF:
#ifdef NFP_NET_THB350
		tx_bar_off = NFP_PCIE_QUEUE(0) +
			NFP_NET_VF_OFFSET(max_tx_rings, PCI_FUNC(pdev->devfn));
		rx_bar_off = NFP_PCIE_QUEUE(128) +
			NFP_NET_VF_OFFSET(max_rx_rings, PCI_FUNC(pdev->devfn));
#else
		tx_bar_off = 0;
		rx_bar_off = 0;
#endif
	...
	if (!err) {
		nn_dbg(nn, "Unable to allocate MSI Vectors. Exiting\n");
		goto err_msi_alloc;
	}
...
static void nfp_netvf_pci_remove(struct pci_dev *pdev)
{
	 ...
	 * to keep the nn pointer around till we have freed
	 * everything. */
	if (nn) {
		nn->removing_pdev = 1;
