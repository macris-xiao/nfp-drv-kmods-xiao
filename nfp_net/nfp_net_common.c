...
/**
 * nfp_net_msix_unmask - Unmask a MSI-X interrupt
 *
 * The firmware is auto-masking a MSI-X and it needs to be un-masked
 * when done with handling the it.  There doesn't seem to be a general
 * function for doing this.
 */
static void nfp_net_msix_unmask(struct nfp_net *nn, unsigned int vec_nr)
{
	int off;

	off = (PCI_MSIX_ENTRY_SIZE * vec_nr) + PCI_MSIX_ENTRY_VECTOR_CTRL;
	/* Make sure all updates are written before re-enabling interrupts */
	wmb();
	nn_writel(nn->msix_table, off, 0);
}

/**
 * nfp_net_msi_unmask - Unmask a MSI interrupt
 *
 * Unmasks MSI interrupt if MSI per-vector masking is enabled.
 *
 * XXX: Not sure yet if FW will be auto-masking MSI and if unmasking will
 *      be enabled
 */
static void nfp_net_msi_unmask(struct nfp_net *nn, unsigned int vec_nr)
{
	...
	if (control & PCI_MSI_FLAGS_MASKBIT) {
		spin_lock_irqsave(&nn->msilock, flags);

		if (control & PCI_MSI_FLAGS_64BIT)
			pci_read_config_dword(pdev,
					      cap + PCI_MSI_MASK_64, &masked);
		else
			pci_read_config_dword(pdev,
					      cap + PCI_MSI_MASK_32, &masked);
		...
		spin_unlock_irqrestore(&nn->msilock, flags);
	}
}

/**
 * nfp_net_irq_unmask - Unmasks MSI/MSI-X interrupt
 *
 * Note, we don't enable auto masking if we have a shared interrupt.
 * In that case this functions becomes a no-op.
 */
static void nfp_net_irq_unmask(struct nfp_net *nn, unsigned int vec_nr)
{
	struct pci_dev *pdev = nn->pdev;
	...
	if (pdev->msi_enabled)
		nfp_net_msi_unmask(nn, vec_nr);
	else if (pdev->msix_enabled)
		nfp_net_msix_unmask(nn, vec_nr);
}
...
/**
 * nfp_net_msi_alloc - Try to allocate MSI irqs
 *
 * For MSI we are OK with 1 or more vectors. Older Linux kernels seem
 * to offer only, even when more are requested.  Also check if per
 * vector masking is supported while at it.
 *
 * Return: Number of MSI vectors obtained or 0 or error.
 */
#if 0 /* Unused for now */
static int nfp_net_msi_alloc(struct nfp_net *nn, int nr_vecs)
{
	...
	nvecs = pci_enable_msi_range(pdev, 1, nr_vecs);
	if (nvecs < 0) {
		nn_err(nn, "Failed to enable MSI. Wanted %d-%d (err=%d)\n",
		       NFP_NET_NON_Q_VECTORS + 1, nr_vecs, nvecs);
		return 0;
	}
	...
	for (i = 0; i < nvecs; i++) {
		nn->irq_entries[i].entry  = i;
		nn->irq_entries[i].vector = pdev->irq + i;
	}
 ...
 *
 * Allocate MSI-X for the PF and MSI for the VF.
 *
...
{
	if (nn->pdev->msi_enabled)
		pci_disable_msi(nn->pdev);
	else if (nn->pdev->msix_enabled)
 ...
 */
int nfp_net_irqs_wanted(struct nfp_net *nn)
{
	int ncpus;
	int vecs;
...
{
	struct nfp_net *nn = netdev_priv(netdev);
	const int len = sizeof(nn->lsc_name);
	...
	snprintf(nn->lsc_name, len, "%s-lsc", netdev->name);

	nn->exn_handler = nfp_net_irq_exn;
	snprintf(nn->exn_name, len, "%s-exn", netdev->name);
	...
	for (i = NFP_NET_NON_Q_VECTORS, r = 0; i < nn->num_vecs; i++, r++) {
		...
		r_vec->irq_idx = i;
		r_vec->requested = 0;
		snprintf(r_vec->name, len, "%s-rxtx-%02d", netdev->name, r);
	...
	for (i = 0; i < txd_cnt; i++) {
		...
		if (tx_ring->txbufs && tx_ring->txbufs[i].dma_addr)
			off += sprintf(p + off, " dma_addr=%#llx",
				       (unsigned long long)tx_ring->txbufs[i].dma_addr);
...
static int nfp_net_rx_fill_freelist(struct nfp_net_rx_ring *rx_ring)
{
	...
	while (nfp_net_rx_space(rx_ring) >= NFP_NET_FL_BATCH) {
		for (i = 0; i < NFP_NET_FL_BATCH; i++) {
			...
			rxd->fld.dd = 0;
			rxd->fld.dma_addr_hi = ((uint64_t)dma_addr >> 32) & 0xff;
...
int nfp_net_rx_dump(struct nfp_net_rx_ring *rx_ring, char *p)
{
	...
	for (i = 0; i < rxd_cnt; i++) {
		...
		if (rx_ring->rxbufs && rx_ring->rxbufs[i].dma_addr)
			off += sprintf(p + off, " dma_addr=%#llx",
				       (unsigned long long)rx_ring->rxbufs[i].dma_addr);
...
static int nfp_net_alloc_resources(struct nfp_net *nn)
{
	...
	/* Force ring memory to low 32bit */
	...
	/* restore DMA mask (this should not fail as it passed in probe()) */
	(void)dma_set_mask(&pdev->dev, DMA_BIT_MASK(NFP_NET_MAX_DMA_BITS));
	return 0;
...
int nfp_net_netdev_init(struct net_device *netdev)
{
	...
	/* Enable MSI Automasking, if supported and the IRQs are not shared.*/
	if (nn->cap & NFP_NET_CFG_CTRL_MSIXAUTO &&
	...
	/* YDS-155 workaround. */
	if (nn->is_nfp3200) {
		err = dma_set_mask(&nn->pdev->dev, DMA_BIT_MASK(32));
		if (err)
			return err;
		nn->spare_va = dma_zalloc_coherent(&nn->pdev->dev, 4096,
						   &nn->spare_dma, GFP_KERNEL);
		if (!nn->spare_va) {
			(void)dma_set_mask(&nn->pdev->dev,
