...
pr_info("%s: %d\n", __func__, __LINE__);
	...
	priv->rtsymtab = kmalloc(priv->numrtsyms * sizeof(struct nfp_rtsym) + 7, GFP_KERNEL);
	if (!priv->rtsymtab) {
		err = -ENOMEM;
		goto err_symtab;
	}
	...
	if (NFP_CPP_MODEL_IS_3200(model)) {
		err = nfp_cpp_read(cpp, dram, priv->mip->symtab_addr,
				   rtsymtab, (priv->mip->symtab_size + 7) & ~7);
...
pr_info("%s: %d\n", __func__, __LINE__);
		...
		err = nfp_cpp_read(cpp, dram, priv->mip->strtab_addr,
				   priv->rtstrtab, (priv->mip->strtab_size + 7) & ~7);
...
pr_info("%s: %d: %d\n", __func__, __LINE__, err);
		...
		for (wptr = (uint32_t *) rtsymtab, n = 0; n < priv->numrtsyms; n++)
		...
		for (n = 0; n < priv->numrtsyms; n++) {
			...
			priv->rtsymtab[n].addr = (((uint64_t) rtsymtab[n].addr_hi) << 32) +
				rtsymtab[n].addr_lo;
			priv->rtsymtab[n].size = (((uint64_t) rtsymtab[n].size_hi) << 32) +
				rtsymtab[n].size_lo;
			...
			case _SYM_TGT_LMEM:
				...
				break;
			case _SYM_TGT_UMEM:
				priv->rtsymtab[n].target = NFP_RTSYM_TARGET_USTORE;
				priv->rtsymtab[n].domain = NFP3200_MELIN2MEID(
					rtsymtab[n].domain1.nfp3200_domain);
				break;
			default:
				priv->rtsymtab[n].target = rtsymtab[n].target;
				priv->rtsymtab[n].domain = rtsymtab[n].domain1.nfp3200_domain;
				break;
			}
		}
		...
		for (n = 0; n < priv->numrtsyms; n++) {
			...
			if (rtsymtab[n].domain2.nfp6000_menum != 0xff)
				...
				priv->rtsymtab[n].domain = rtsymtab[n].domain1.nfp6000_island;
			else
...
{
	struct nfp_rtsym_priv *priv = nfp_device_private(dev, nfp_rtsym_priv_con);
	int err;
