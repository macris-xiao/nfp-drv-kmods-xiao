...
static int nfp6000_spi_csr_readl(struct nfp_spi *spi, uint32_t csr, uint32_t *val)
{
    return nfp_cpp_area_readl(spi->csr, csr, val);
}

static int nfp6000_spi_csr_writel(struct nfp_spi *spi, uint32_t csr, uint32_t val)
{
                ...
                int mdio_data_drive_disable)
{
    ...
    ctrl |= NFP_SPI_SPIPORTMC_DATAWIDTH(spi->width) | spi->clkdiv | CS_BITS(cs);
    ...
    for ( ; (tx_bit_cnt > 0) || (rx_bit_cnt > 0); ) {
        ...
        txbits = min_t(uint32_t, SPI_MAX_BITS_PER_CTRL_WRITE, tx_bit_cnt);
        rxbits = min_t(uint32_t, SPI_MAX_BITS_PER_CTRL_WRITE, rx_bit_cnt);
        ...
        if (txbits) {
            if (txbits % 8)
                _tx[txbits/8] |= ((1 << (8-(txbits%8))) - 1);
            for (i = 0, tmp = 0; i < BITS_TO_BYTES(txbits); i++, _tx++)
                tmp |= (_tx[0] << (24 - (i * 8)));
            for (; i < BITS_TO_BYTES(SPI_MAX_BITS_PER_CTRL_WRITE); i++, _tx++)
                tmp |= (0xff << (24 - (i * 8)));
        } else {
            tmp = 0xffffffff;
        }
        err = nfp6000_spi_csr_writel(spi, NFP_SPI_SPIPORTMDO(spi->bus), tmp);
        if (err < 0)
            return err;
        ...
        if (rxbits) {
            err = nfp6000_spi_csr_readl(spi, NFP_SPI_SPIPORTMDI(spi->bus), &tmp);
            if (err < 0)
                return err;
            if (clk_bit_cnt < SPI_MAX_BITS_PER_CTRL_WRITE)
                tmp = tmp << (SPI_MAX_BITS_PER_CTRL_WRITE - clk_bit_cnt);
...
struct nfp_spi *nfp_spi_acquire(struct nfp_device *nfp, int bus, int width)
{
    ...
    for (; timeout > 0; timeout -= 100) {
        nfp6000_spi_csr_writel(spi, NFP_SPI_SPIWRITEENABLETARGET, spi->key);
