				...
				struct netlink_ext_ack *extack)
{
	...
	if (dissector_uses_key(flow->dissector, FLOW_DISSECTOR_KEY_CVLAN)) {
		struct flow_dissector_key_vlan *flow_cvlan;
		...
		flow_cvlan = skb_flow_dissector_target(flow->dissector,
...
#if VER_NON_RHEL_LT(5, 8) || VER_RHEL_LT(8, 3)
...
#endif /* VER_NON_RHEL_LT(5, 8) || VER_RHEL_LT(8, 3)*/
