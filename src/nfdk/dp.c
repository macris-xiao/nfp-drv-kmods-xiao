		...
		struct sk_buff *skb)
{
	...
	/* TODO decide how to handle md_bytes
	 * The driver doesn't have it easily available at this point.
...
netdev_tx_t nfp_nfdk_tx(struct sk_buff *skb, struct net_device *netdev)
{
	...
	if (__netdev_tx_sent_queue(nd_q, real_len, skb_xmit_more(skb)))
		nfp_net_tx_xmit_more_flush(tx_ring);
...
{
	/* TODO */
	nfp_nfdk_rx_drop(dp, rx_ring->r_vec, rx_ring, rxbuf, NULL);
	return false;
}
#endif
...
bool __nfp_nfdk_ctrl_tx(struct nfp_net *nn, struct sk_buff *skb)
{
...
bool nfp_nfdk_ctrl_tx(struct nfp_net *nn, struct sk_buff *skb)
{
	struct nfp_net_r_vector *r_vec = &nn->r_vecs[0];
	bool ret;
