	       ...
	       struct sk_buff *skb, u64 *tls_handle, int *nr_frags)
{
	...
	if (unlikely(resync_pending || ntls->next_seq != seq)) {
		...
		if (!resync_pending && seq - ntls->next_seq < U32_MAX / 4)
			compat__tls_offload_tx_resync_request(nskb->sk, seq,
							      ntls->next_seq);
...
const struct net_device_ops nfp_nfd3_netdev_ops = {
	...
	.ndo_size		= sizeof(nfp_nfd3_netdev_ops),
	.extended		= {
		.ndo_set_vf_trust	= nfp_app_set_vf_trust,
#if VER_RHEL_GE(7, 4)
		.ndo_set_vf_vlan	= nfp_app_set_vf_vlan,
...
void nfp_net_info(struct nfp_net *nn)
{
	...
	nn_info(nn, "VER: %d.%d.%d.%d, Maximum supported MTU: %d\n",
		nn->fw_ver.dp_resv, nn->fw_ver.class,
	      ...
	      unsigned int max_tx_rings, unsigned int max_rx_rings)
{
	...
	switch (FIELD_GET(NFP_NET_CFG_VERSION_DP, nn->fw_ver.dp_resv)) {
