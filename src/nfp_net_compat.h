...
{
#if defined(COMPAT__HAVE_IP6_DST_FLOW_LOOKUP)
...
#if VER_NON_RHEL_GE(5, 9)
#define compat__nfp_flower_indr_setup_tc_cb nfp_flower_indr_setup_tc_cb
#elif VER_NON_RHEL_GE(5, 8) || RHEL_RELEASE_GE(8, 305, 0, 0)
...
static inline int
compat__nfp_flower_setup_indr_tc_release(enum tc_setup_type type,
					 void *type_data, void *cb_priv)
{
	return nfp_flower_setup_indr_block_cb(type, type_data, cb_priv);
}
#endif
...
#if (VER_NON_RHEL_GE(5, 3) && VER_NON_RHEL_LT(5, 8)) || VER_RHEL_GE(8, 2)
static inline void
