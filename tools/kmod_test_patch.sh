...
(
	...
	(
	    ...
	    #
	    nl_warnings=$(grep -nrI '[^n]"[,)]' src/ | sed -e '/nfp_pr_et/d;/"AS IS"/d;/MODULE_/d;/sn*printf/d;/scanf/d;/_phymod_get_attr_/d;/? ".*" *: ".*"/d;/seq_/d;/strncpy/d;/debugfs_create_/d;/NL_SET_ERR_MSG_MOD/d;/_rtsym.*(/d' | wc -l)
